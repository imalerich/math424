...
void print_vec(int * vec, int n); // used to print iterations
...
double Trap(double a, double b, int n, int thread_count) {
	...
	// stores which thread modified our data
	...
	for (i = 1; i <= n-1; i++) {
		...
		// set thread number which computed this iterationn
		iterations[i] = omp_get_thread_num();
	}

	approx = h*approx; 

	print_vec(iterations, n);
	free(iterations);
	return approx;
}  /* Trap */
